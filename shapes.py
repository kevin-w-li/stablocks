...
def make_pile_given_noise(space=None, base_coord = [(0., 100.), (500., 100.)], base_width = 10,  mass = 1,
                          block_dim = [100,40], noise_list=[0] * 5, hor_ver_list = [0] * 5):
    if space is None:
        space = []
    ...
    for i in range(num_of_blocks):
        ...
        trunc_sample = noise_list[i] * var * x_range + last_block_pos[0]
...
def make_pile(space=None, num_of_blocks=5,base_coord = [(0., 100.), (500., 100.)], base_width = 10,
                          block_dim = [100,40], noise = 0.4):
    if space is None:
        space = []
    add_base(space, base_coord[0], base_coord[1], width=base_width)
    first_block_pos = [(base_coord[0][0] + base_coord[1][0]) / 2., base_coord[0][1] + base_width + block_dim[1] / 2.]
    ...
    for i in range(num_of_blocks):
        ...
        x_range = last_block_width / 2. + perm_block_dim[0] / 2.
        trunc_sample = truncnorm.rvs(- 1. / (noise), 1. / (noise), size=num_of_blocks) * noise * x_range + last_block_pos[0]
        last_block_pos = [trunc_sample , last_top + perm_block_dim[1] / 2.]
        last_top = last_block_pos[1] + perm_block_dim[1] / 2.
        body_list.append(body)
        shape_list.append(shape)
